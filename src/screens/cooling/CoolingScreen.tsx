...
export const CoolingScreen: React.FC = () => {
  ...
  // Animations - same as treatment screen
  const fillAnim = useRef(new Animated.Value(1)).current; // 1 -> 0 (empties down)
  const iconPulse = useRef(new Animated.Value(1)).current;

  const circleSize = Math.max(screenWidth, screenHeight) * 3.5;

  useEffect(() => {
    // Emptying fill animation - LINEAR for constant speed
    Animated.timing(fillAnim, {
      toValue: 0,
      duration: DURATION,
      easing: Easing.linear, // Linear = constant speed, no acceleration
    ...
    // Countdown synced to duration (like treatment screen)
  ...
  // Map fillAnim (1→0) to height (circleSize→0)
  // fillAnim starts at 1 (full), goes to 0 (empty)
  // fillHeight should start at circleSize (full), go to 0 (empty)
  const fillHeight = fillAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, circleSize], // When fillAnim=1, height=circleSize; when fillAnim=0, height=0
  });
...
const styles = StyleSheet.create({
  ...
  coolIcon: { width: 120, height: 120, tintColor: '#3B82F6' },
  countdownText: {
    marginTop: 6,
    fontSize: 40,
    fontWeight: '700',
